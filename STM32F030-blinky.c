...
//  Attach an anode (+ side) of LED to GPIO Pin A0. Then connect a 1k resister between the
...
int main( void ){
  ...
  GPIOA->MODER |= ( GPIO_MODER_MODER6_0);   // Set CNF0[1] and MODE0[1] bits.
  
//  GPIOA->AFR[0] |= (1<<GPIO_AFRL_AFSEL6_Pos);
//  GPIOA->AFR[1] |= (1<<GPIO_AFRL_AFSEL6_Pos);
